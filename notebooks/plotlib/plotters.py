...
def plot_single_orientation(
    ...
    for m in metric_names:
        for i, df in enumerate(dfs):
            ...
            if m.startswith('rbp@'):
                es = 'rbp-res@'+m[4:]
                axs[cnt].fill_between(x, [y[m] for y in df], [y[es]+y[m] for y in df], alpha=alpha)

        axs[cnt].set_ylabel(metric_names[m],fontsize=30)

        axs[cnt].tick_params(labelsize=20)
        axs[cnt].yaxis.set_major_formatter(FormatStrFormatter('%.2f'))
        cnt += 1 
